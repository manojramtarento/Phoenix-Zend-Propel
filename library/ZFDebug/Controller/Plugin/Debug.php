...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract {
	 ...
	 * @var array
	 */
	public static $standardPlugins = array('Cache', 'Html', 'Database', 'Exception', 'File', 'Memory', 'Registry', 'Time', 'Variables', 'Propel');
	...
	protected function _output($html) {
		...
		if (Zend_Layout::getMvcInstance()->isEnabled()) {
			$response->appendBody($this->_headerOutput());
			$response->appendBody('<div id="ZFDebug_debug">' . $html . '</div>');
		}
	}
