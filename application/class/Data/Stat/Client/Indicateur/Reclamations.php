 ...
 * @author info20
 */
class Data_Stat_Client_Indicateur_Reclamations extends Data_Stat_Client_Indicateur_Abstract {
    ...
    public function getTitle() {
        return Phoenix_Translation_Files::getPdf($this->getStatClientInstance()->getOperation()->getOpDevisLang(), 'stats', 'nb_demandes');
    }
    ...
    public function addNodeXmlToDoc(\Phoenix_Stats_Xml_Builder &$oPhoenix_Stats_Xml_Builder) {
        ...
        $oPhoenix_Stats_Xml_Builder->addNbDemandes($data);
    }

    public function addToExcel(\PHPExcel &$oPHPExcel) {
        $aData = $this->getData();
        $sheet = $oPHPExcel->createSheet();
        $sheet->setCellValueByColumnAndRow(0, 1, 'Réclamations');
